...
function pushState() {
  // Push a deep copy of currentPuzzle onto the undo stack
...
redoBtn.addEventListener("click", () => {
  ...
  // Push current state onto undo stack before redoing
  undoStack.push(JSON.parse(JSON.stringify(currentPuzzle)));
...
submitButton.addEventListener("click", async () => {
  ...
  try {
    ...
    // Collapse the input area so that focus is on the puzzle state
    document.getElementById("input-method").classList.add("hidden");
    manualInputDiv.classList.add("hidden");
    pasteInputDiv.classList.add("hidden");
    uploadInputDiv.classList.add("hidden");
    startTimer();
    updateRemainingDigits();
    // Clear history stacks since this is a new puzzle.
    undoStack = [];
    redoStack = [];
    updateHistoryButtons();
  } catch (error) {
...
// Render puzzle state as interactive HTML table
